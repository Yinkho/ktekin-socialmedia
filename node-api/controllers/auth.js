    ...
    User.findOne({ email }, (err, user) => {
        ...
        if (err || !user) {
            return res.status(401).json({
                error: 'Cet email ne correspond à aucun utilisateur. Veuillez vous inscrire.'
            });
        }
...
exports.forgotPassword = (req, res) => {
    ...
    User.findOne({ email }, (err, user) => {
        ...
        if (err || !user)
            return res.status('401').json({
                error: 'Cet email ne correspond à aucun utilisateur !'
            });
        ...
        const emailData = {
            ...
            html: `<p>Veuillez cliquer sur le lien suivant pour réinitialiser votre mot de passe :</p> <p>${
                process.env.CLIENT_URL
        ...
        return user.updateOne({ resetPasswordLink: token }, (err, success) => {
            if (err) {
                ...
                return res.status(200).json({
                    message: `Un email a été envoyé à ${email}. Veuillez suivre les instructions.`
                });
            }
        });
    });
...
exports.resetPassword = (req, res) => {
    ...
    User.findOne({ resetPasswordLink }, (err, user) => {
        ...
        user.save((err, result) => {
            ...
            res.json({
                message: `Félicitation, votre mot de passe a été mis à jour.`
            });
        });
    });
};
